...
impl<'a> ComputationEngine<'a> {
    ...
    pub fn compute_canonical_collection(mut self, diag_info: &mut DiagnosticInfo) -> CanonicalCollection {
        ...
        while let Some((state_id, items)) = worklist.pop() {
            ...
            diag_info.dump_closure(self.interned_symbols, &self.context, state_id, &items);
